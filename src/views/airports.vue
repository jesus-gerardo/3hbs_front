        ...
        <v-dialog v-model="dialog" persistent transition="dialog-bottom-transition" width="500"> 
            <v-card>
                <v-toolbar dark dense color="primary">
                    ...
                    <v-toolbar-title>{{titleDialog}} airport</v-toolbar-title>
                    <v-spacer></v-spacer>
                </v-toolbar>

                <v-card-text>
                    <v-form ref="formAirports">
                        <v-row>
                            <v-col cols="12">
                                <v-text-field
...
export default {
    ...
    data(){
        ...
        async load(){
            try{
                this.overlay = true;
                let {data} = await this.$http.get("/airports");
                this.overlay = false;
        ...
        close(){
            ...
            if(this.$refs.formAirports){
                this.$refs.formAirports.resetValidation();
            }
            this.dialog = false;
        },
        async register(){
            try{
                let valid = this.$refs.formAirports.validate();
                if(!valid){
                    return false
                }
                this.overlay = true;
                let {data} = await this.$http.post('/airports', this.formData);
                this.overlay = false;
                if(!data.success){
                    console.error("error");
                    return;
        ...
        async update(){
            try{
                ...
                this.overlay = true;
                let {data} = await this.$http.put(`/airports/${this.formData.id}`, this.formData);
                this.overlay = false;
                if(!data.success){
                    console.error("error");
                    return;
        ...
        async destroy(row){
            try{
                this.overlay = true;
                let {data} = await this.$http.delete(`/airports/${row.id}`);
                this.overlay = false;
                if(!data.success){
                    console.error("error");
                    return;
